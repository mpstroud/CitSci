...
```{r,warning=F}
#necessary packages: 
pack_list<-list ("ggplot2","openair","dygraphs","xts","dataRetrieval","dplyr","plyr","tidyr","forecast","lubridate", "expss","gganimate","mgcv","mvtnorm","statmod")
...
AllDataSummarized_byHour.forGAM = temp.summary %>% 
  filter(site %in% c("PUCC2S.csv")) %>%
  arrange(month, day, hour) %>%
  mutate(temp = mean) %>%
  select(day,hour,month,temp)
AllDataSummarized_byHour.forGAM$c.day=row.names(AllDataSummarized_byHour.forGAM) #rownames as cumulative time factor
GAM_data <- AllDataSummarized_byHour.forGAM #simplify name


gam1<- gam(temp ~ s(day), data = GAM_data, method = 'REML')

gam1<- gam(temp ~ mgmc::s(day, bs="cc") + s(c.day, bs='cr',k=300), 
           data=AllDataSummarized_byHour.forGAM, method = 'REML', 
